...
#pragma simd
	for (int flow_i = 0; flow_i < flow_end; flow_i++)
	{
		...
		NUMERIC_TYPE channel_effective_width = C(0.0);
			...
			h1 + bankFullHeight1 > depth_thresh)
		{
			...
			if (hflow > depth_thresh)
			{
				...
				if (channel_area0 < channel_area1) // select the appropriate slope and area based on the smallest area
				{
					// calculate hydraulic radius for SGC
					R = SGC2_CalcR(channel_group0, hflow, bankFullHeight0, channel_width0, sg_pair_SGC_width[flow_pair_index], channel_area0, SGCptr);
					area = channel_area0;
					channel_effective_width = channel_width0; // set effective width

				}
				else
				{
					// calculate hydraulic radius for SGC
					R = SGC2_CalcR(channel_group1, hflow, bankFullHeight1, channel_width1, sg_pair_SGC_width[flow_pair_index_next], channel_area1, SGCptr);
					area = channel_area1;
					channel_effective_width = channel_width1; // set effective width
				}

				channel_ratio = channel_effective_width / cell_width;
		...
		sg_flow_ChannelRatio[flow_index] = getmin(channel_ratio, C(1.0));
	}


	//this update is moved to a separate loop so that the CalculateQ can be vectorized
#pragma ivdep
	for (int flow_i = 0; flow_i < flow_end; flow_i++)
	{
		int flow_index = sg_row_start_index + flow_i;
		...
		NUMERIC_TYPE channel_Q = sg_flow_Q[flow_index];
		NUMERIC_TYPE channel_ratio = sg_flow_ChannelRatio[flow_index];

		//note this check only works for D4, needs to be updated for D8
		if (grid_index0 == grid_index1 - 1) // qx
		{
			if (channel_ratio < C(1.0))
			{
				Qx_grid[grid_index1] = Qx_old_grid[grid_index1] - channel_ratio * Qx_old_grid[grid_index1];
			}
			else
			{
				Qx_grid[grid_index1] = C(0.0);
				Qx_old_grid[grid_index1] = C(0.0);
			}
			Qx_grid[grid_index1] += channel_Q;
		}
		else if (grid_index0 == grid_index1 - grid_cols_padded) // qy
		{
			if (channel_ratio < C(1.0))
			{
				Qy_grid[grid_index1] = Qy_old_grid[grid_index1] - channel_ratio * Qy_old_grid[grid_index1];
			}
			else
			{
				Qy_grid[grid_index1] = C(0.0);
				Qy_old_grid[grid_index1] = C(0.0);
			}
			Qy_grid[grid_index1] += channel_Q;
		}
		else
		{
			printf("d8\n");
		}
		if (channel_Q != C(0.0) || channel_ratio > C(0.0))
		{
...
inline void SGC2_UpdateQx_row(const int grid_cols,
	...
	const NUMERIC_TYPE depth_thresh, const NUMERIC_TYPE row_dx, const NUMERIC_TYPE row_dy,
...
#pragma ivdep
		for (int cell_i = 0; cell_i < cell_count; cell_i++)
		{
			...
			//	flow_dV -= (flow_index != -1) ? sg_flow_ChannelRatio[flow_index] * Qx_grid[grid_index] : C(0.0);

			//}
			//	{
			//		// 0 qx
			//		int flow_index = sg_cell_flow_lookup_item.flow_subtract[0];
			//		flow_dV -= (flow_index != -1) ? sg_flow_Q[flow_index] : C(0.0);
			//		flow_dV += (flow_index != -1) ? sg_flow_ChannelRatio[flow_index] * Qx_grid[grid_index + 1] : C(0.0);
			//	}

			//	{
			//		// 1 is qy
			//		int flow_index = sg_cell_flow_lookup_item.flow_add[1];
			//		flow_dV += (flow_index != -1) ? sg_flow_Q[flow_index] : C(0.0);
			//		flow_dV -= (flow_index != -1) ? sg_flow_ChannelRatio[flow_index] * Qy_grid[grid_index] : C(0.0);
			//	}
			//	{
			//		// 1 is qy
			//		int flow_index = sg_cell_flow_lookup_item.flow_subtract[1];
			//		flow_dV -= (flow_index != -1) ? sg_flow_Q[flow_index] : C(0.0);
			//		flow_dV += (flow_index != -1) ? sg_flow_ChannelRatio[flow_index] * Qy_grid[grid_index + grid_cols_padded] : C(0.0);
			//	}
			//d8 add more flows here

			//flow_dV *= delta_time;
			//dV += flow_dV;
			volume_grid[grid_index] += dV;
		}
	}
	...
	VolumeHeightUpdateInfo * update_info)
{
	...
	for (int j = start_y; j < end_y; j++)
	{
		...
		block_evap_loss += SGC2_UpdateVol_sub_grid_row(sg_row_start, cell_row_count, grid_cols, grid_cols_padded,
			...
			sg_cell_cell_area, sg_cell_flow_lookup, sg_flow_Q, sg_flow_ChannelRatio, Qx_grid, Qy_grid, sg_cell_SGC_group, sg_cell_SGC_c, sg_cell_SGC_is_large,
			volume_grid, h_grid, evap_grid, wet_dry_bounds, SGCptr);
