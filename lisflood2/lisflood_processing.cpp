...
void CopyToSubSubGridFlowInfo(int grid_index, int grid_index_next, int flow_index, NUMERIC_TYPE g,
	...
	NUMERIC_TYPE grid_cell_width, // grid_cell_width cell width perpendicular to flow direction (todo for d8 have both dimentions) 
	SubGridFlowInfo * flow_info, Pars *Parptr, Arrays *Arrptr, SGCprams *SGCptr)
{
	...
	flow_info->sg_flow_cell_width[flow_index] = grid_cell_width;
}
...
void CopyToSubGridCellInfo(int grid_cols_padded, int cell_index, int x, int y, SubGridCellInfo * sub_grid_cell_info, Pars *Parptr, Arrays *Arrptr, SGCprams *SGCptr)
{
	...
	sub_grid_cell_info->sg_cell_SGC_is_large[cell_index] = (sub_grid_cell_info->sg_cell_SGC_width[cell_index] >= C(0.5)*(Arrptr->dx[source_row_index] + Arrptr->dy[source_row_index]));
}

int CheckSubGrid(const int source_index_this, const int i, const int j, const int grid_rows, const int grid_cols,
	const Arrays * Arrptr, const int weirs_enable)
{
	...
	if (i < (grid_cols - 1) &&
		...
		(weirs_enable != ON || Arrptr->Weir_Identx[source_index_qx] == -1)
		) // don't add the sub-grid calculation where there is a weir
	{
		//printf("sgx at %d, %d\n", i, j);
		this_flow_count++;
	}
	if ((j < grid_rows - 1))
	{
		if ((Arrptr->SGCwidth[source_index_below] > C(0.0)) &&
			...
			(weirs_enable != ON || Arrptr->Weir_Identy[source_index_qy] == -1)
			) // don't add the sub-grid calculation where there is a weir
		{
			//printf("sgy at %d, %d\n", i, j);
			this_flow_count++;
...
#pragma novector
		...
		for (int i = 0; i < grid_cols; i++)
		{
			...
			{
				int this_flow_count;
				this_flow_count = CheckSubGrid(source_index_this, i, j, grid_rows, grid_cols, Arrptr, Statesptr->weirs);
...
#pragma novector
	...
	sub_grid_state->sg_flow_ChannelRatio = (NUMERIC_TYPE*)memory_allocate(flow_memory_size * sizeof(NUMERIC_TYPE));
	...
	sub_grid_flow_info->sg_flow_cell_width = (NUMERIC_TYPE*)memory_allocate(flow_memory_size * sizeof(NUMERIC_TYPE));
	...
	for (int bi = 0; bi < block_count; bi++)
	{
		...
		for (int j = row_range[bi].start; j < row_range[bi].end; j++)
		{
			...
			for (int i = 0; i < Parptr->xsz; i++)
			{
				...
				if (Arrptr->SGCwidth[source_index_this] > C(0.0) && (Arrptr->DEM[source_index_this] != DEM_NO_DATA || Arrptr->ChanMask[source_index_this] > 0))
				{
					int this_flow_count = CheckSubGrid(source_index_this, i, j, grid_rows, Parptr->xsz, Arrptr, Statesptr->weirs);
...
void InitSuperLinksStructure(const int grid_rows, const int grid_cols, const int grid_cols_padded, SuperGridLinksList * Super_linksptr, States *Statesptr, Pars *Parptr, Arrays *Arrptr, SGCprams *SGCptr, Solver * Solverptr, Fnames * Fnameptr, int verbose)
{
	...
	{
		// Step 0: Create temp variables
		int max_n_links = Parptr->ysz*Parptr->xsz;
		int * tmp_link_index_SGC_i = new int[max_n_links]();
		int * tmp_link_index_SGC_j = new int[max_n_links]();
		int * tmp_link_index_2D_i = new int[max_n_links]();
		int * tmp_link_index_2D_j = new int[max_n_links]();
		NUMERIC_TYPE * tmp_z_SGC = new NUMERIC_TYPE[max_n_links]();
		NUMERIC_TYPE * tmp_z_2D = new NUMERIC_TYPE[max_n_links]();
		NUMERIC_TYPE * tmp_gn2 = new NUMERIC_TYPE[max_n_links]();
		NUMERIC_TYPE * tmp_w = new NUMERIC_TYPE[max_n_links]();
		NUMERIC_TYPE * tmp_dx = new NUMERIC_TYPE[max_n_links]();
		NUMERIC_TYPE * tmp_bfH = new NUMERIC_TYPE[max_n_links]();

		// Step 1: Loop around ChanMask idetify locations of SGC and edges
		...
		for (i = 0; i < n_links; i++)
		{
			...
			for (j = 1; j < n_SGC_under_mask; j++) // loop through sub-grid cells
			{
				...
				if (dist > dist2)
				{
					closest_i = tmp_link_index_SGC_i[j]; closest_j = tmp_link_index_SGC_j[j]; closest_line = j;
					dist = dist2;
				}
			}
