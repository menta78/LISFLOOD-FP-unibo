...
)
{
    ...
    zero_thin_depth_slopes(Parptr, Solverptr, Arrptr);
...
)
{
	...
	update_Hstar(Parptr, Arrptr, U_int);
...
)
{
	...
	case FREE1:
		break;
	case HFIX2:
		H_inside = H_outside = FMAX(C(0.0), BCptr->BC_Val[bc_i] - Zstar);
		break;
	case HVAR3:
		H_inside = H_outside = FMAX(C(0.0), linear_interpolate(
...
)
{
    ...
    if (thin_depth(Solverptr, U.H[j*Parptr->xsz + i]))
    {
        U_inc.H1x = C(0.0);
        U_inc.HU1x = C(0.0);
        U_inc.HV1x = C(0.0);
...
)
{
    ...
    else if (H > Solverptr->DepthThresh)
	{
		NUMERIC_TYPE U = HU/H;
		NUMERIC_TYPE V = HV/H;
