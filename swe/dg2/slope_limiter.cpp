...
#pragma omp parallel for
	for (int j=0; j<Parptr->ysz; j++)
	{
		...
		{
			const int i = 0;
			U.H1x[j*Parptr->xsz + i] = -Arrptr->DEM1x[j*Parptr->xsz + i];
...
#pragma omp parallel for
	for (int i=0; i<Parptr->xsz; i++)
	{
		...
		{
			const int j = 0;
			U.H1y[j*Parptr->ysz + i] = -Arrptr->DEM1y[j*Parptr->xsz + i];
            U.HU1y[j*Parptr->ysz + i] = C(0.0);
            U.HV1y[j*Parptr->ysz + i] = C(0.0);
		}
